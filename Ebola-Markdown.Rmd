...
As it has been done in [Section 2.2](#Out1.2), in order to check if the chains has reached the stationary, different diagnostics also here have been performed.
...
It is also possible to take a look at the __Gelman-Rubin Plot__ for the estimated probabilities $p_{i}$ by __FIgure 13__:
...
We cannot only rely on the stationarity, we need also here to check autocorrelation. In order to take a look at the the values of the autocorrelation values them selves by using the autocorr.diag() function in coda. What we expect is that the autocorrelation does not persist for longer once we move forward in the iterations. As it possible to see from the output below for the coefficients $\beta_{s}$ and estimated $p_{i}$ probabilities the autocorrelation doesn't drop very fast, but it takes some Lags. 
