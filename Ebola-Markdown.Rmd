...
In this first model independence among each death probabilities has been assumed among each sub-area. In order to model the outcome, a Binomial distribution has been used. The total number of infected people per each sub area $i$ as been denoted with $n_{i}$. The total number of deceased people is represented by $r_{i}$ which is a binary response variable with "true" probability $p$. 

\[r_i \sim Binomial(p,n_i) \] 

A $Beta(0.5,0.5)$ distribution as been used as standard-non-informative prior for $p$. The Beta distribution is a continuous distribution taking values in the domain 0-1.

\[p_i \sim Beta(0.5, 0.5) \] 

In order to make inference on the $p$ the model has been written in order to be compatible for JAGS (Just Another Gibbs Sampler).
...
Do we rely on these output? In order to check it, it is possible to use the Bayesian Conjugate Model:
...
convergence of the chains, we could rely on a better diagnostic called
...
Values faraway from 1 indicates that the chains haven't reached yet convergence.
...
As can be seen from the JAGS output, all the four $\beta_{s}$ parameters seems to be approximately normal (as shown in __Figure 13__) with a very low variance and different from zero. This means that they should be reliable and have actually an impact on estimating the probability of death.

As it is also possible to seen from the output table returned by JAGS, the estimated death probabilities per each Area quite high. But where are they come from? Since it has been used a Binomial with a prob of success $p_{i}$ and the logit of each $p_{i}$ has been modeled as a linear model, this leads to an exponential form equal to:
...
Since Trace PLots provide an informal diagnostic about the convergence of the chains, we could rely on a better diagnostic called __Gelman & Rubin diagnostic__. This diagnostic calculates the variability within/inside the chains and compares that to the variability between the chains (Gelman & Rubin). We can actually compute this diagnostic by calling the gelman.diag() funciton implemented in coda library. The Potential Scale Factor is the statistics that tells us about the chain convergence should be close to 1. Values farway from 1 indicates that the chains haven’t reached yet convergence.
...
Looking at the DIC values, it is clearly shown that there is a significant difference among them. For that reason, it is possible to assess that Model 2 is preferable over Model 1. In fact, the DIC difference is equal to:
