...
### <span style="color: CornflowerBlue;"> 2.1) JAGS Model 1 </span>
...
However, since the plot is still too confused, it could help our diagnostic to have a closer look to it by using the _"window"_ parameter inside the _"mcmc_trace function"_ and see how every chain behaves. 
...
__ Gelman & Rubin diagnostic__. This diagnostic calculates the variability
...
It shows how the "_shrinkage_ factor" changes once we add iterations to the chains.
...
### <span style="color: CornflowerBlue;"> 2.4) Inferential finding Model 1 </span>
...
actually only make the model more complex. For that reason only __GAM__ has been included since it has a stronger correlation with the dependent variable compared to the other two.
...
### <span style="color: CornflowerBlue;"> 3.1) JAGS Model 2 </span>
...
### <span style="color: CornflowerBlue;"> 3.2) Output & Convergence Diagnostic </span>
...
#### <span style="color: turquoise;"> 3.2.1) TracePlot & Gelman-Rubin </span>
...
can be seen from __Figure 9__, all the chains seem to be exploring the same region of parameter values and this can be infer by the fact that all the chains lie on top of each other. 
...
```
Instead, in the animated figure below it is possible to see the __Traceplots__ for the estimated probabilities $p_{i}$:

![__$p_{i}$ TracePlot animated GIF__](images/pi_traceplot.gif)
...
As __Figure 10__ shows, it seems that the chains spend substantial time in the same region of the parameter space, sign that they are reaching stationarity.


```{r message=FALSE, include=TRUE, eval=TRUE, class.source="bg-success", warning=FALSE, fig.cap = "Figure 10: Trace Plot"}
mcmc_trace(chainArray2, pars = "p[2]", window = c(300,500),
           facet_args = list(labeller = ggplot2::label_parsed))+
  myfacets
```

Another relevant diagnostic analysis can be done by checking the autocorrelation plots. It is possible to visualize the autocorrelation by calling the __mcmc_acf__ function. This function shows the autocorrelation for each Markov chain separately up to a used-defined number of lags for each preselected parameters. What is expected is to see a low autocorrelation since it is a signal of convergence. As it possible to see from the __Figure 11__, the autocorrelation function drops to 0 pretty fast after only a couple of lags.
...
Another diagnostic  analysis can be done by taking a look at the plots of estimated density of the parameters and of the deviance, where density plots are just smoothed histograms of the samples. These are shown in __Figure 12__ reported here below.

```{r message=FALSE, include=TRUE, eval=TRUE, class.source="bg-success", warning=FALSE, fig.cap = "Figure 12: Density Overlay Plot"}
# (3) mcmc_density
color_scheme_set("pink")
## (3.1) plot first 4 p_{i}
myfacets <-
...
```{r message=FALSE, include=TRUE, eval=TRUE, class.source="bg-success", warning=FALSE, fig.cap = "Figure 13: Density Overlay Plot"}
# (3) mcmc_density
color_scheme_set("pink")
## (3.1) plot first 4 p_{i}
myfacets <-
...
![__$p_{i}$ Density Plot animated GIF__](images/pi_density.gif)

It is also possible to have a look at the posterior distribution of the parameters as shown in __Figures 14 and 15__

```{r message=FALSE, include=TRUE, eval=TRUE, class.source="bg-success", warning=FALSE, fig.cap = "Figure 14: Posterior Density Plot"}
# (4) areas plot
# (4.1) areas plot
color_scheme_set("green")
plot_title <- ggtitle("Posterior distributions",
...
```{r message=FALSE, include=TRUE, eval=TRUE, class.source="bg-success", warning=FALSE, fig.cap = "Figure 15: Deviance Posterior Density Plot"}
## (4.2) deviance area
mcmc_areas(chainArray2,
           pars=c("deviance"),
           prob = 0.9, point_est = 'median') +
  ggtitle("Devinace Posterior distribution", "with medians & 90% intervals")
```

To have a look at the distribution it is also possible to use the violin plot also implemented in the bayesplot library.

```{r message=FALSE, include=TRUE, eval=TRUE, class.source="bg-success", warning=FALSE, fig.cap = "Figure 16: Violin Plot"}
# (4) mcmc_violin
color_scheme_set("green")
## (4.1) plot first 4 p_{i}
mcmc_violin(chainArray2, par=c("beta[1]","beta[2]","beta[3]","beta[4]"),
            facet_args = list(labeller = ggplot2::label_parsed),
            probs = c(0.1, 0.5, 0.9)) + 
...
### <span style="color: CornflowerBlue;"> 3.3) Inferential finding Model 2 </span>
...
p.hat.jags <- colMeans(chainMatrix2)
p.hat.jags
```
...
```{r message=FALSE, include=TRUE, eval=TRUE, class.source="bg-success", warning=FALSE}
cred <- 0.95
p.ET.jags <- apply(chainMatrix2, 2, quantile, prob=c((1-cred)/2, 1-(1-cred)/2))
p.ET.jags
```

__HPD Intervals: __
```{r message=FALSE, include=TRUE, eval=TRUE, class.source="bg-success", warning=FALSE}
p.HPD.jags <- coda::HPDinterval(as.mcmc(chainMatrix2))
p.HPD.jags
```


## <span style="color: purple;"> 3)  Model Comparison </span>
...
```
Looking at the DIC values, it is clearly shown that there is a significant diffence among them. And in fact the difference is equal to:
